...
public:
	...
	const VkPhysicalDevice GetPhysDevice() const { return m_PhysDevice; };
...
inline std::pair<VkImage, VkDeviceMemory> GP2GraphicsPipeline<V>::CreateTextureImage(const std::string& fileName, const GP2CommandPool& commandPool, const VkQueue& queue)
{
	...
	CreateImage(m_Device, m_PhysDevice, texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage, textureImageMemory);

	TransitionImageLayout(m_Device, textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, commandPool, queue);
	...
	TransitionImageLayout(m_Device, textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, commandPool, queue);
...
{
	VkCommandBuffer commandBuffer = BeginSingleTimeCommands(m_Device, commandPool);
	...
	EndSingleTimeCommands(m_Device, commandBuffer, commandPool, queue);
}
